const poster = {
  ...
  drawPoster(canvasId, winWidth, winHeight, imgs, text, name, callback) {
    ...
    for (let i = 0; i < row.length; i++) {
      context.fillText(
        row[i],
        (winWidth - w / 2) / 2,
        (winHeight + uni.upx2px(460)) / 2 + a * i
      );
    }
    // 识别小程序二维码
  ...
  judgePermissionPhotoLibrary: async function (callback) {
    ...
    if (isAndroid) {
      result = await poster.requestAndroidPermission(
...
export default {
  drawShipInfoPoster: poster.drawShipInfoPoster,
