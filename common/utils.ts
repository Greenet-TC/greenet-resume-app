...
export const getCategoryTreeToArr = (category, level) => {
  const arr: ICategoryType[] = []
  getTreeItem(category, level, arr)
  return arr
}

function getTreeItem(data, level, arr) {
  data.forEach((item) => {
    if (item.category === level) {
      arr.push(item) // 结果赋值
    } else {
      if (item.innerTree) {
        getTreeItem(item.innerTree, level, arr)
      }
    }
  })
}
